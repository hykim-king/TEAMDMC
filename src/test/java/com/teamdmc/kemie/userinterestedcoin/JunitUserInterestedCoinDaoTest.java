...
public class JunitUserInterestedCoinDaoTest {
	...
	public void doUpdate() throws SQLException{
		...
		dao.doDelete(uic02);
		dao.doDelete(uic03);
		assertEquals(0, dao.getCount(uic01));
		
		// [2] 데이터  등록
		dao.doInsert(uic01);
		dao.doInsert(uic02);
		dao.doInsert(uic03);
		assertEquals(3, dao.getCount(uic01));
...
//		searchVO.setSearchWord("tubus");
		List<UserInterestedVO> list = dao.doRetrieve(searchVO);
		for(UserInterestedVO vo: list) LOG.debug("vo= "+vo);
	}
